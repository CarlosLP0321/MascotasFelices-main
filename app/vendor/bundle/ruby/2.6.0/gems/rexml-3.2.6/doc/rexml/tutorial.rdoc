...
{duenos.xml}[https://www.w3schools.com/xml/duenos.xml] at w3schools.com.
...
For convenience, we capture the XML into a string variable:
  ...
  require 'open-uri'
  source_string = URI.open('https://www.w3schools.com/xml/duenos.xml').read
...
so the source can be from a web page:
  ...
  require 'open-uri'
  io = URI.open("https://www.w3schools.com/xml/duenos.xml")
  io.class # => StringIO
  doc = Document.new(io)
...
[XPath]
  ...
  relative to its most distant ancestor:
    ...
    doc.root.xpath             # => "/duenostore"
    doc.root.texts.first       # => "\n\n"
    doc.root.texts.first.xpath # => "/duenostore/text()"
...
[Parent]
  ...
  Use inherited method REXML::Child#parent to retrieve the parent
    ...
    ele = doc.root                # => <duenostore> ... </>
    ele.parent                    # => <UNDEFINED> ... </>
    ele = doc.root.elements.first # => <book category='cooking'> ... </>
    ele.parent                    # => <duenostore> ... </>
...
[Element Children]
  ...
  containing the element children:
    ...
    eles      # => #<REXML::Elements:0x000001ee2848e960 @element=<duenostore> ... </>>
    eles.size # => 4
